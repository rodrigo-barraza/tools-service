...
NAS_COMPOSE_DIR="/volume1/docker/tools-api"   # Synology path where compose lives
NAS_SMB_DIR="/mnt/k/tools-api"           # Fallback: Windows SMB mount
...
echo -e "${CYAN}${BOLD}  🔧 Tools API — Deploy to Synology${RESET}"
if $DRY_RUN; then
...
if $HAS_SSH; then
  ...
  if $DRY_RUN; then
    ...
    info "  2. Project → tools-api → Stop → Start"
  fi
fi
